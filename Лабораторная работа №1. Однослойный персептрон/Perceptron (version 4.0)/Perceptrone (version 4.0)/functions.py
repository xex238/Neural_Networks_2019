...
def learning_function(inputs, outputs, weights, speed, activation_function, MSE_mas, accuracy_mas, count_of_neurons):
    ...
    for j in range(len(inputs)):
        ...
        for i in range(count_of_neurons): # len(synaptic_weights = кол-во столбцов входных данных)
            ...
            sum.append(scalar(inputs[j], value_synaptic_weights))
            if(activation_function == "softsign"):
                output.append(softsign_activation_function(sum[i]))
            if(activation_function == "sigmoid"):
                output.append(sigmoid_activation_function(sum[i]))
            if(i == outputs[j]):
        ...
        if(activation_function == "softsign"):
            for i in range(len(weights)):
                for k in range(len(weights[0])):
                    weights[i][k] = weights[i][k] + speed * err[k] * derivative_softsign_activation_function(sum[k]) * inputs[j][i]
                    #weights[i][k] = weights[i][k] + speed * err[k] * derivative_softsign_activation_function(output[k]) * inputs[j][i]
                    #weights[i][k] = weights[i][k] + speed * err[k] * inputs[j][i]
        if(activation_function == "sigmoid"):
            for i in range(len(weights)):
                for k in range(len(weights[0])):
                    weights[i][k] = weights[i][k] + speed * err[k] * derivative_sigmoid_activation_function(sum[k]) * inputs[j][i]
                    #weights[i][k] = weights[i][k] + speed * err[k] * derivative_sigmoid_activation_function(output[k]) * inputs[j][i]
                    #weights[i][k] = weights[i][k] + speed * err[k] * inputs[j][i]
    MSE = MSE / len(inputs)
    MSE_mas.append(MSE)
